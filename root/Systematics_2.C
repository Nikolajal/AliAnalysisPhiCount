...
{
    Int_t   nOptions    =   12;
    string  sOptions[]  =   {"CH3","CH5","M","W","R1","R2","R3","R4","R5","R6","R7","R8"};
    Int_t   nOption2    =   9;
    string  sOption2[]  =   {"R1","R2","R3","R4","R5","R6","R7","R8","BK"};
    ...
    h1D_Systematics_Util    ->  Divide(h1D_Syst[2],h1D_Raw);
    h1D_Systematics_Util    ->  Write();

    
    TH1F   *fh1D_FnlBin  = new   TH1F ("Final_Syst",        "Final_Syst",       nBinPT1D-4,4,nBinPT1D);
    TH1F   *fh1D_FnlBiM  = new   TH1F ("Final_Syst_Mean",   "Final_Syst_Mean",  nBinPT1D-4,4,nBinPT1D);
    TH1F   *fh1D_FnlBiW  = new   TH1F ("Final_Syst_Widt",   "Final_Syst_Widt",  nBinPT1D-4,4,nBinPT1D);
    TH1F   *fh1D_FnGBin  = new   TH1F ("FinaG_Syst",        "FinaG_Syst",       nBinPT1D-4,4,nBinPT1D);
    TH1F   *fh1D_FnGBiM  = new   TH1F ("FinaG_Syst_Mean",   "FinaG_Syst_Mean",  nBinPT1D-4,4,nBinPT1D);
    TH1F   *fh1D_FnGBiW  = new   TH1F ("FinaG_Syst_Widt",   "FinaG_Syst_Widt",  nBinPT1D-4,4,nBinPT1D);
    for ( Int_t iTer = 4; iTer < nBinPT1D; iTer++ )
    {
        fh1D_Systematics_Bin[iTer]->GetXaxis()->SetRangeUser(0.8,1.2);
        fh1D_FnlBiM     ->  SetBinContent   (iTer-3,  fabs(1- fh1D_Systematics_Bin[iTer]->GetMean()) );
        fh1D_FnlBiW     ->  SetBinContent   (iTer-3,  fabs(fh1D_Systematics_Bin[iTer]->GetRMS()) );
        fh1D_FnlBin     ->  SetBinContent   (iTer-3,  fabs(1- fh1D_Systematics_Bin[iTer]->GetMean()) + fabs(fh1D_Systematics_Bin[iTer]->GetRMS()));
        TF1* func = new TF1 ("func","gaus",0.5,1.5);
        fh1D_Systematics_Bin[iTer]  ->  Fit(func,"IMREQ0S");
        fh1D_FnGBiM     ->  SetBinContent   (iTer-3,  fabs(1- func->GetParameter(1)) );
        fh1D_FnGBiW     ->  SetBinContent   (iTer-3,  fabs(func->GetParameter(2))    );
        fh1D_FnGBin     ->  SetBinContent   (iTer-3,  fabs(1- func->GetParameter(1)) + fabs(func->GetParameter(2)));
        fh1D_Systematics_Bin[iTer]->GetXaxis()->SetRangeUser(0.1,1.9);
        fh1D_Systematics_Bin[iTer]->Write();
    }
    TH1F   *fh1D_FnlTT_   = new   TH1F("Total1D",           "Total1D",          100,0.,1.);
    TH1F   *fh1D_FnlTTM   = new   TH1F("Total1D_mean",      "Total1D_mean",     100,0.,1.);
    TH1F   *fh1D_FnlTTW   = new   TH1F("Total1D_widt",      "Total1D_widt",     100,0.,1.);
    for ( Int_t iTer = 4; iTer < nBinPT1D; iTer++ )
    {
        fh1D_FnlTT_->Fill(fh1D_FnlBin->GetBinContent(iTer-3));
        fh1D_FnlTTM->Fill(fh1D_FnlBiM->GetBinContent(iTer-3));
        fh1D_FnlTTW->Fill(fh1D_FnlBiW->GetBinContent(iTer-3));
    }
    ...
    for ( Int_t iFll = 0; iFll < nOptions; iFll++ )
    {
        if ( sOptions[iFll] == "R2" ) continue;
        if ( sOptions[iFll] == "R4" ) continue;
        if ( sOptions[iFll] == "R6" ) continue;
        if ( sOptions[iFll] == "R7" ) continue;
        if ( sOptions[iFll] == "R8" ) continue;
    ...
    for ( Int_t iTer = 2; iTer < nBinPT2D; iTer++ )
    {
        for ( Int_t jTer = iTer; jTer < nBinPT2D; jTer++ )
        {

...
\title{A simple {\it ALICE analysis\/} journal} 
...
{Nicola Rubini$^{1\ast}$\\
\\
\normalsize{$^{1}$Department of Physics, University of Bologna}\\
\\
\normalsize{$^\ast$To whom correspondence should be addressed; E-mail:  nicola.rubini@cern.ch}
}
...
\section{February 12$^{th}$ 2021}
To make the multiplicity optional, so as to make the run over 7TeV dataset quicker and in general the analysis more modular, one could make a class that take in input a name and a file, so as to name the output file accordingly and only fit/process the given data in the given way. This can be easily generalised with the use of a for cycle over the multiplicity bins from the preprocessing. Speaking of pre-processing it can be useful to to separate the fils with fit results/ invariant mass histograms in different files. Maybe it can be thoughtful to keep 1D and 2D analysis together in the processing chain as they are 2 parts of a unique measurement. One could also unify the p$_{T}$ binning in 1D and 2D as per the same reason so as to not inroduce further systematic error in the chain. One could then take the 1D fit as a benchmark for the fit shapes.\\
\textbf{New feature:} There should be a QA attached to the analysis chain that automatically checks for the consistency of 2D-1D efficiency, together with inter dataset/runlist comparison for efficiencies and raw yield (maybe only for 1D inclusive? 2D inclusive?)
\textbf{Check the inclusive yield:} there is an inconsistency between what you published on your thesis and what you get now. Try to recover waht you published or check with the \textbf{updated paper} published spectra, so taht you can hope to have a chance at doing something meaningful.
...
TODO			&DONE\\
\hline
 A lot, as always:	&Nothing, as always:\\
1. Re-Run analysis Task using the new set-up, excluding the multiplicity and pile-up check to have a consistent check with thesis-published data\\
2. Adapt the Analysis task to have a tree on event selection with structure \{iEvent, fEventMask, if (kMCbool) fEventTrueFlag\} \\
2.a) Adapt the Analysis task to have on all data trees a leaf with iEvent to have an offline selection \\
2.b) Check the AliEventCuts methodology for vertex and general event selection \\
2.c) Have multiple flags: e.g. Vertex cut 9-10-11cm for systematic evaluation\\
\end{tabularx}
\end{table}

(... yes, there are 3 days of nothing inbetween ...)

\newpage
\section{February 16$^{th}$ 2021}
...
\hline
 A lot, as always:																		&Nothing, as always:\\
1. Do the automatic check for the efficiency 1Dx1D over the 2D for all analyses configurations.\\
2. Pythia configurations can be made in .cmnd files: make all configurations in files so as to slim your main\\
3. \\
4. \\
5. \\
& \\

...
( TString fOption = "Yield", TString kFolder = "_p_p__7TeV" )    {
    ...
    if ( kDoYield ) {
        ...
        for ( auto kCurrent_Syst : kSyst_TRK_XD_Options ) {
            push_to_front( k1D_VarInFiles, new TFile ( Form(kASigExtp_FitCheckRst,(TString("Yield")+kFolder+TString("/Systematics/TRK/")+(kCurrent_Syst)).Data(),"1D") ) );
        ...
        for ( auto kCurrent_Syst : kSyst_TRK_XD_Options ) {
            push_to_front( k2D_VarInFiles, new TFile ( Form(kASigExtp_FitCheckRst,(TString("Yield")+kFolder+TString("/Systematics/TRK/")+(kCurrent_Syst)).Data(),"2D") ) );
        ...
        uSysEvaluate_Extrapolation_Custom1D  ( h1D_Nraw_stat, k1D_Variations, h2D_Nraw_stat, k2D_Variations, TString(Form(kAnalysis_Systemt_Dir,(TString("Yield")+kFolder).Data()))+TString("TRK"), "", false );
    }
}

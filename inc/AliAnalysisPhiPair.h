...
//
//>>    Analysis Utility
#include "/Users/nrubini/Analysis/AnUtils/AliAnalysisUtility/AliAnalysisUtility.h"
//
...
Float_t
fMeasureMeanPT(TF1 *fLowFit, TGraphAsymmErrors *gTotal, Bool_t fReFit = false); //! TODO: TO BE CLEAN
template <class Tclass>
Float_t fEvaluateMeanPT(Tclass *fTotalUncertaintySpectrum, TF1 *fLowPTModel = nullptr, Bool_t kRequestReFit = false)
{
    //
    //  Result
    Float_t fResult = 0;
    ...
    //
    //  Re-Fit if requested
    if (kRequestReFit && fLowPTModel)
        fTotalUncertaintySpectrum->Fit(fLowPTModel, "IMEQS");
    //
...
template <class Tclass>
void fEvaluateMeanPT(Tclass *fTotalUncertaintySpectrum, Double_t &kError)
{
    //
    //  Result
    Float_t fResult = 0;
...
{
    if (!fOption.compare("InvMass1D"))
        return 1;
    if (!fOption.compare("RapTru"))
        return 1;
    if (!fOption.compare("Rap"))
        return 3;
    if (!fOption.compare("xInvMass2D"))
        return 2;
    if (!fOption.compare("yInvMass2D"))
        return 2;
    else
        return -1;
}
...
//
RooFitResult *
fFitCoreModel(TH1 *THdata, TH1F *hSlopReference, TString fName = "", TString fOption = "", Int_t PTindex = -1, Int_t PTDimension = 1, TString fPathToSave = "./result/SEFitCheck")
{
    //
    ...
    RooRealVar ch1 = RooRealVar("ch1", "ch1", 0.5, -1, 1);
    RooRealVar ch2 = RooRealVar("ch2", "ch2", -.06, -1, 1);
    RooRealVar ch3 = RooRealVar("ch3", "ch3", kNullCh3 * 0.01, kNullCh3 * -1, kNullCh3 * 1);
    RooRealVar ch4 = RooRealVar("ch4", "ch4", kNullCh4 * 0., kNullCh4 * -1, kNullCh4 * 1);
    if (fDegree2 && !fDegree4)
    {
        ch3.setConstant(kTRUE);
        ch4.setConstant(kTRUE);
    }
    if (!fDegree2 && !fDegree4)
    {
        ch4.setConstant(kTRUE);
    ...
    auto kPythiaSlop = bPythiaTest ? 0. : 1.;
    auto kWidtLoose = 10.;
    auto kWidtRef = fRescaleRes * hSlopReference->GetBinContent(PTindex + 1);
    RooRealVar sMass = RooRealVar("bMass", "bMass", kPhiMesonMass_, kPhiMesonMass_ * 0.5, kPhiMesonMass_ * 1.5);
    RooRealVar sWidt = RooRealVar("bWidt", "bWidt", kPhiMesonWidth, kPhiMesonWidth * 0.1, kPhiMesonWidth * 10.);
    RooRealVar sSlop = RooRealVar("bSlop", "bSlop", kPythiaSlop * kWidtRef, kPythiaSlop * kWidtRef / kWidtLoose, kPythiaSlop * kWidtRef * kWidtLoose);
    if (!fUseFreeRes)
        sSlop.setConstant(kTRUE);
    if (!fLosWidt)
        sWidt.setConstant(kTRUE);
    //
    //  --- Normalisation coefficients
    RooRealVar nSS = RooRealVar("anSS", "anSS", .5 * nEntries, 0., nEntries);
    RooRealVar nBB = RooRealVar("anBB", "anBB", .5 * nEntries, 0., nEntries);
    //
    //  --- Building the PDFs
    ...
    for (Int_t iCycle = 0; iCycle < kNCycle; iCycle++)
    {
        if (!kOnlyTrue)
        {
            ...
            if (fIsResultAcceptable(N_Raw->getVal(), N_Raw->getError()))
                break;
        }
        if (kOnlyTrue)
        {
    ...
    if (fSaveToFile || kSaveToFile)
    {
        ...
        {
            if (PTDimension == 1)
                hTitle = Form("Invariant Mass of Kaons in pT %.1f-%.1f GeV", fArrPT1D[PTindex], fArrPT1D[PTindex + 1]);
            if (PTDimension == 2)
                hTitle = Form("Invariant Mass of Kaons in pT %.1f-%.1f GeV", fArrPT2D[PTindex], fArrPT2D[PTindex + 1]);
        }
        if (PTindex != -1 && bPythiaTest)
        {
            if (PTDimension == 1)
                hTitle = Form("Invariant Mass of Kaons in pT %.1f-%.1f GeV for MC", fArrPT1D[PTindex], fArrPT1D[PTindex + 1]);
            if (PTDimension == 2)
                hTitle = Form("Invariant Mass of Kaons in pT %.1f-%.1f GeV for MC", fArrPT2D[PTindex], fArrPT2D[PTindex + 1]);
        }
        ...
        if (PTDimension == 1)
            fSaveToFrame->GetYaxis()->SetTitle(Form("Counts/( %.1f MeV/#it{c}^{2} )", 1000 * kBinningPrecision1D));
        if (PTDimension == 2)
        ...
        if (is_pp_anl)
            uLatex->DrawLatexNDC(0.59, 0.77, Form("pp #sqrt{#it{s}}= %.2f TeV", kEnergy));
        if (is_pb_anl)
            uLatex->DrawLatexNDC(0.59, 0.77, Form("pPb #sqrt{#it{s}}= %.2f TeV", kEnergy));
        if (PTDimension == 1)
            uLatex->DrawLatexNDC(0.59, 0.72, Form("%.2f < #it{p}_{T} < %.2f GeV/#it{c}", fArrPT1D[PTindex], fArrPT1D[PTindex + 1]));
        if (PTDimension == 2)
            uLatex->DrawLatexNDC(0.59, 0.72, Form("%.2f < #it{p}_{T} < %.2f GeV/#it{c}", fArrPT2D[PTindex], fArrPT2D[PTindex + 1]));
        uLatex->DrawLatexNDC(0.59, 0.67, "#phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5");

        fSaveToCanvas->Write();
        if (PTDimension == 1)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s.pdf", fPathToSave.Data(), fArrPT1D[PTindex], fArrPT1D[PTindex + 1], fName.Data()));
        if (PTDimension == 2)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s.pdf", fPathToSave.Data(), fArrPT2D[PTindex], fArrPT2D[PTindex + 1], fName.Data()));
        if (PTDimension == 1)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s.eps", fPathToSave.Data(), fArrPT1D[PTindex], fArrPT1D[PTindex + 1], fName.Data()));
        if (PTDimension == 2)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s.eps", fPathToSave.Data(), fArrPT2D[PTindex], fArrPT2D[PTindex + 1], fName.Data()));
        ...
        //
        if (PTDimension == 1)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s_Chi2.pdf", fPathToSave.Data(), fArrPT1D[PTindex], fArrPT1D[PTindex + 1], fName.Data()));
        if (PTDimension == 2)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s_Chi2.pdf", fPathToSave.Data(), fArrPT2D[PTindex], fArrPT2D[PTindex + 1], fName.Data()));
        if (PTDimension == 1)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s_Chi2.eps", fPathToSave.Data(), fArrPT1D[PTindex], fArrPT1D[PTindex + 1], fName.Data()));
        if (PTDimension == 2)
            fSaveToCanvas->SaveAs(Form("%s/PT_%.1f_%.1f_1D_%s_Chi2.eps", fPathToSave.Data(), fArrPT2D[PTindex], fArrPT2D[PTindex + 1], fName.Data()));
        delete fSaveToCanvas;
    }
    //
...
//
std::vector<TH1F *>
FitModel(TH1F **hTarget, TH1F *hSlopReference, RooFitResult **&fFitresultsStore, Int_t fDimension, TString fOption = "", TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "")
{
    std::vector<TH1F *> fResults;
    if (!fFitresultsStore)
    {
        fFitresultsStore = new RooFitResult *[((fDimension == 1) ? nBinPT1D : nBinPT2D)];
    }
    for (Int_t iFit = 0; iFit < ((fDimension == 1) ? nBinPT1D : nBinPT2D); iFit++)
    {
        ...
        if (iFit == 0)
        {
            for (auto fCoeff : fFitresultsStore[iFit]->floatParsFinal())
            {
                ...
                if (fDimension == 1)
                    fResults.push_back(new TH1F(Form("%s%s_%s", "h1D_", N_Raw->GetName(), fNameFile.Data()), Form("%s%s", "h1D_", N_Raw->GetName()), nBinPT1D, fArrPT1D));
                else
                    fResults.push_back(new TH1F(Form("%s%s_%s", "h2Dbin_", N_Raw->GetName(), fNameFile.Data()), Form("%s%s", "h2Dbin_", N_Raw->GetName()), nBinPT2D, fArrPT2D));
            }
...
std::vector<TH1F *>
FitModel(TH1F **hTarget, TH1F *hSlopReference, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", TString fOption = "")
{
    return FitModel(hTarget, hSlopReference, NULL_ROOFITPTR2, 1, fOption, fTargetPath, fNameFile);
}
std::vector<TH1F *>
FitModel(TH1F **hTarget, TH1F *hSlopReference, RooFitResult **&fFitresultsStore, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", TString fOption = "")
{
    return FitModel(hTarget, hSlopReference, fFitresultsStore, 2, fOption, fTargetPath, fNameFile);
}
//
std::vector<TH1F *>
FitModel(std::vector<TH1F *> hTarget, TH1F *hSlopReference, RooFitResult **&fFitresultsStore, Int_t fDimension, TString fOption = "", TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "")
{
    std::vector<TH1F *> fResults;
    if (!fFitresultsStore)
    {
    ...
    for (auto kCurrent_Target : hTarget)
    {
        ...
        if (iFit == 0)
        {
            for (auto fCoeff : fFitresultsStore[iFit]->floatParsFinal())
            {
                ...
                if (fDimension == 1)
                    ...
                    fResults.push_back(new TH1F(Form("%s%s_%s", "h1D_2Db_", N_Raw->GetName(), fNameFile.Data()), Form("%s%s", "h1D_2Db_", N_Raw->GetName()), nBinPT2D, fArrPT2D));
            }
            for (auto fCoeff : fFitresultsStore[iFit]->constPars())
            {
                auto N_Raw = static_cast<RooRealVar *>(fCoeff);
        ...
        for (auto fCoeff : fFitresultsStore[iFit]->floatParsFinal())
        {
            ...
            if (strncmp(N_Raw->GetName(), "bMass", 5) == 0)
                fMean = N_Raw->getVal();
            if (strncmp(N_Raw->GetName(), "bWidt", 5) == 0)
                fWidth = N_Raw->getVal();
            if (strncmp(N_Raw->GetName(), "bSlop", 5) == 0)
                fResl = N_Raw->getVal();
            iTer++;
        }
        for (auto fCoeff : fFitresultsStore[iFit]->constPars())
...
std::vector<TH1F *>
FitModel(std::vector<TH1F *> hTarget, TH1F *hSlopReference, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", TString fOption = "")
{
    return FitModel(hTarget, hSlopReference, NULL_ROOFITPTR2, 1, fOption, fTargetPath, fNameFile);
}
std::vector<TH1F *>
FitModel(std::vector<TH1F *> hTarget, TH1F *hSlopReference, RooFitResult **&fFitresultsStore, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", TString fOption = "")
{
    return FitModel(hTarget, hSlopReference, fFitresultsStore, 2, fOption, fTargetPath, fNameFile);
}
//
...
//
RooFitResult *
fFitCoreModel(TH2F *THdata, TH1F *hSlopReference, RooFitResult *fFitShapeX, RooFitResult *fFitShapeY, TString fName = "", TString fOption = "", Int_t PTindex = -1, Int_t PTjndex = -1, TString fPathToSave = "./result/SEFitCheck")
{
    //
    //>>    Silencing TCanvas Pop-Up and speeding Fit procedures
    gROOT->SetBatch(kTRUE);
    ...
    if (!fIsWorthFitting(THdata) || fAlwaysFit)
        return nullptr;
    //
    //>>    Global Variables
    Double_t fInvMassValMax, fInvMassValMin;
    ...
    //
    //>>    Background
    RooRealVar ch1x = RooRealVar("ch1x", "ch1x", fXShapes->getRealValue("ch1", 0), fXShapes->getRealValue("ch1", 0), fXShapes->getRealValue("ch1", 0));
    RooRealVar ch2x = RooRealVar("ch2x", "ch2x", fXShapes->getRealValue("ch2", 0), fXShapes->getRealValue("ch2", 0), fXShapes->getRealValue("ch2", 0));
    RooRealVar ch3x = RooRealVar("ch3x", "ch3x", fXShapes->getRealValue("ch3", 0), fXShapes->getRealValue("ch3", 0), fXShapes->getRealValue("ch3", 0));
    RooRealVar ch4x = RooRealVar("ch4x", "ch4x", fXShapes->getRealValue("ch4", 0), fXShapes->getRealValue("ch4", 0), fXShapes->getRealValue("ch4", 0));
    RooRealVar ch1y = RooRealVar("ch1y", "ch1y", fYShapes->getRealValue("ch1", 0), fYShapes->getRealValue("ch1", 0), fYShapes->getRealValue("ch1", 0));
    RooRealVar ch2y = RooRealVar("ch2y", "ch2y", fYShapes->getRealValue("ch2", 0), fYShapes->getRealValue("ch2", 0), fYShapes->getRealValue("ch2", 0));
    RooRealVar ch3y = RooRealVar("ch3y", "ch3y", fYShapes->getRealValue("ch3", 0), fYShapes->getRealValue("ch3", 0), fYShapes->getRealValue("ch3", 0));
    RooRealVar ch4y = RooRealVar("ch4y", "ch4y", fYShapes->getRealValue("ch4", 0), fYShapes->getRealValue("ch4", 0), fYShapes->getRealValue("ch4", 0));
    if (!fFreeBkg)
    {
        ch1x.setConstant(kTRUE);
        ch2x.setConstant(kTRUE);
        ch3x.setConstant(kTRUE);
        ch4x.setConstant(kTRUE);
        ch1y.setConstant(kTRUE);
        ch2y.setConstant(kTRUE);
        ch3y.setConstant(kTRUE);
        ch4y.setConstant(kTRUE);
    }
    ...
    for (Int_t iCycle = 0; iCycle < kNCycle; iCycle++)
    {
        ...
        if (fIsResultAcceptable(N_Raw->getVal(), N_Raw->getError(), k2DErrorLimit))
            break;
    }
    // Save to file
    if (fSaveToFile || kSaveToFile)
    {
        ...
        for (int i = 0; i < nBinsPrint; i++)
        {
            ...
            if (PTindex != -1 && !bPythiaTest)
                hTitle = Form("Slice of 2D Invariant Mass of Kaons in pT %.1f-%.1f GeV, %.1f-%.1f GeV", fArrPT2D[PTindex], fArrPT2D[PTindex + 1], fArrPT2D[PTjndex], fArrPT2D[PTjndex + 1]);
            if (PTindex != -1 && bPythiaTest)
                hTitle = Form("Slice of 2D Invariant Mass of Kaons in pT %.1f-%.1f GeV, %.1f-%.1f GeV for MC", fArrPT2D[PTindex], fArrPT2D[PTindex + 1], fArrPT2D[PTjndex], fArrPT2D[PTjndex + 1]);
            ...
            if (is_pp_anl)
                uLatex->DrawLatexNDC(0.50, 0.77, Form("pp #sqrt{#it{s}}= %.2f TeV, #phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5", kEnergy));
            if (is_pb_anl)
                uLatex->DrawLatexNDC(0.50, 0.77, Form("pPb #sqrt{#it{s}}= %.2f TeV, #phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5", kEnergy));
            ...
            if (i == 0)
            {
                ...
                if (is_pp_anl)
                    uLatex->DrawLatexNDC(0.55, 0.77, Form("pp #sqrt{#it{s}}= %.2f TeV, #phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5", kEnergy));
                if (is_pb_anl)
                    uLatex->DrawLatexNDC(0.55, 0.77, Form("pPb #sqrt{#it{s}}= %.2f TeV, #phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5", kEnergy));
                uLatex->DrawLatexNDC(0.55, 0.72, Form("%.2f < #it{p}_{T,#phi_{1}} < %.2f GeV/#it{c}", fArrPT2D[PTindex], fArrPT2D[PTindex + 1]));
                uLatex->DrawLatexNDC(0.56, 0.67, Form("%.2f < #it{p}_{T,#phi_{2}} < %.2f GeV/#it{c}", fArrPT2D[PTjndex], fArrPT2D[PTjndex + 1]));
                uLatex->DrawLatexNDC(0.55, 0.62, Form("%.3f < M_{KK,#phi_{1}} < %.3f GeV/#it{c}^{2}", fInvMassValMin + dIncrement * i, fInvMassValMin + dIncrement * (i + 1)));
            }
...
//
TCanvas *
uPlotSpectrum(TH1 *hTarget, TH1 *hTrSyst, TString fOption = "")
{
    //
    SetStyle();
    //
    ...
    if (fOption.Contains("R"))
        lLegend = new TLegend(0.65, 0.35, 0.85, 0.5);
    else if (fOption.Contains("T"))
        lLegend = new TLegend(0.18, 0.18, 0.38, 0.33);
    else
        lLegend = new TLegend(0.2, 0.35, 0.4, 0.5);
    ...
    if (fOption.Contains("R"))
    {
        uLatex->SetTextFont(60);
        uLatex->SetTextSize(0.05);
        uLatex->DrawLatexNDC(0.65, 0.3, "ALICE Preliminary");
        uLatex->SetTextFont(42);
        uLatex->SetTextSize(0.04);
    ...
    else if (fOption.Contains("T"))
    {
        uLatex->SetTextFont(60);
        uLatex->SetTextSize(0.05);
        uLatex->DrawLatexNDC(0.63, 0.83, "ALICE Preliminary");
        uLatex->SetTextFont(42);
        uLatex->SetTextSize(0.04);
    ...
    else
    {
        uLatex->SetTextFont(60);
        uLatex->SetTextSize(0.05);
        uLatex->DrawLatexNDC(0.20, 0.3, "ALICE Preliminary");
        uLatex->SetTextFont(42);
        uLatex->SetTextSize(0.04);
    ...
    if (fOption.Contains(" 2D"))
    {
        ...
        uLatex->SetTextFont(60);
        uLatex->SetTextSize(0.05);
        uLatex->DrawLatexNDC(0.12, 0.95, "ALICE Preliminary");
        uLatex->SetTextFont(42);
        uLatex->SetTextSize(0.04);
...
//
std::vector<TH2F *>
FitModel(std::vector<TH1F *> hShapeFit, TH1F *hSlopReference, std::vector<std::vector<TH2F *>> hTarget, RooFitResult ***&fFitresultsStore = NULL_ROOFITPTR3, TString fOption = "", TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", std::vector<TH1F *> &f1Din2DbinCheck = NULL_VECTOR)
{
...
std::vector<TH2F *>
FitModel(std::vector<std::vector<TH2F *>> hTarget, TH1F *hSlopReference, RooFitResult **fShapeStore, RooFitResult ***&fFitresultsStore = NULL_ROOFITPTR3, TString fOption = "", TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", std::vector<TH1F *> &f1Din2DbinCheck = NULL_VECTOR)
{
    cout << "cdddddddd" << endl;
    cout << "cdddddddd" << endl;
    cout << "cdddddddd" << endl;
        ...
        for (Int_t jFit = 0; jFit < nBinPT2D; jFit++)
        {
            ...
            if (iFit == 0 && jFit == 0)
            {
                ...
                for (auto fCoeff : fFitresultsStore[iFit][jFit]->floatParsFinal())
                {
                    ...
                    if (strncmp(N_Raw->GetName(), "anSS2D", 6) == 0)
                        fResults.at(iTer)->SetName("hRAW_2D");
                    iTer++;
                }
            }
            //
...
std::vector<TH2F *>
FitModel(std::vector<TH1F *> hShapeFit, TH1F *hSlopReference, std::vector<std::vector<TH2F *>> hTarget, std::vector<TH1F *> &f1Din2DbinCheck, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", TString fOption = "")
{
    return FitModel(hShapeFit, hSlopReference, hTarget, NULL_ROOFITPTR3, fOption, fTargetPath, fNameFile, f1Din2DbinCheck);
}
std::vector<TH2F *>
FitModel(std::vector<std::vector<TH2F *>> hTarget, TH1F *hSlopReference, RooFitResult **fShapeStore, std::vector<TH1F *> &f1Din2DbinCheck, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "")
{
    return FitModel(hTarget, hSlopReference, fShapeStore, NULL_ROOFITPTR3, "", fTargetPath, fNameFile, f1Din2DbinCheck);
}
std::vector<TH2F *>
FitModel(std::vector<std::vector<TH2F *>> hTarget, TH1F *hSlopReference, RooFitResult **fShapeStore, std::vector<TH1F *> &f1Din2DbinCheck, TString fTargetPath = "./result/SEFitCheck", TString fNameFile = "", TString fOption = "")
{
...
Double_t HistIntegrals(Tclass *aTarget, string aOption = "", Double_t aLowBound = 0, Double_t aHigBound = 0)
{
    ...
    {
        fPower = true;
        fnPowr = aOption.at(aOption.find("x^") + 2) - '0';
    }
    else
        fPower = false;
    ...
    for (Int_t fiTer = 1; fiTer <= aTarget->GetNbinsX(); fiTer++)
    {
        ...
        if (aTarget->GetBinLowEdge(fiTer) <= aLowBound && aLowBound != aHigBound)
            continue;
        if (aTarget->GetBinLowEdge(fiTer + 1) >= aHigBound && aLowBound != aHigBound)
            break;

        Double_t fCycleAdd = (aTarget->GetBinContent(fiTer));
        if (fError)
            fCycleAdd = (aTarget->GetBinError(fiTer));
        if (fWidth)
            fCycleAdd *= (aTarget->GetBinWidth(fiTer));
        if (fPower)
            fCycleAdd *= pow((aTarget->GetBinCenter(fiTer)), fnPowr);
        if (fError)
            fCycleAdd *= fCycleAdd;
        fResult += fCycleAdd;
    }
...
fChooseOption(TString fOption)
{
    ...
    //  Check option: SPHEROCITY
    for (auto kString : kOptStrings_Spherocity)
    {
        if (fOption.Contains(kString, TString::kIgnoreCase))
        {
            kDoSpherocity = true;
    ...
    if (kDoYield)
        cout << "[INFO] Yield option chosen" << endl;
    if (kDoMultiplicity)
        cout << "[INFO] Multiplicity option chosen" << endl;
    if (kDoCorrelation)
        cout << "[INFO] Correlation option chosen" << endl;
    if (kDoSpherocity)
        cout << "[INFO] Spherocity option chosen" << endl;
    auto fResult = kDoYield || kDoMultiplicity || kDoCorrelation || kDoSpherocity;
    if (!fResult)
        cout << "[INFO] No option chosen" << endl;
    return fResult;
}
...
void fSetFunction(TF1 *fFitFunction = fLevyTsallis, Double_t fIntegral = 0.032)
{
    ...
    if (nEnne != -1)
    {
        ...
        {
            fFitFunction->SetParLimits(nEnne, 2.1, 12.);
            fFitFunction->SetParameter(nEnne, 7.);
        }
        if (strncmp(fFitFunction->GetName(), "PowerLaw", 13) == 0)
            fFitFunction->SetParameter(nEnne, 13);
    }
    //
    //____________________________________________dN/dy
    ...
    if (nSlop != -1)
    {
        ...
        if (strncmp(fFitFunction->GetName(), "LevyTsallis", 11) == 0)
            fFitFunction->SetParameter(nSlop, .300);
        if (strncmp(fFitFunction->GetName(), "MTExponential", 13) == 0)
            fFitFunction->SetParameter(nSlop, .560);
        if (strncmp(fFitFunction->GetName(), "PTExponential", 13) == 0)
            fFitFunction->SetParameter(nSlop, .560);
        if (strncmp(fFitFunction->GetName(), "FermiDirac", 13) == 0)
            fFitFunction->SetParameter(nSlop, .330);
        if (strncmp(fFitFunction->GetName(), "Boltzmann", 13) == 0)
            fFitFunction->SetParameter(nSlop, .350);
        if (strncmp(fFitFunction->GetName(), "BoseEinstein", 13) == 0)
            fFitFunction->SetParameter(nSlop, .350);
        if (strncmp(fFitFunction->GetName(), "PowerLaw", 13) == 0)
            fFitFunction->SetParameter(nSlop, .550);
    }
}
...
THXTarget_Type *
fSetSystErrors(THXTarget_Type *hStatistics, TString kSystematicsFile, TString kSystematicHisto = "hFullSystematic1D")
{
    //
    ...
    {
        cout << Form("[WARNING] File %s not found", kSystematicsFile.Data()) << endl;
        return fResult;
    }
    //
    for (Int_t iBin = 1; iBin <= fResult->GetNbinsX(); iBin++)
    {
        for (Int_t jBin = 1; jBin <= fResult->GetNbinsY(); jBin++)
        {
            for (Int_t kBin = 1; kBin <= fResult->GetNbinsZ(); kBin++)
            {
                ...
                auto kBinError = 0.;
                if (hReference)
                    kBinError = kBinContent * hReference->GetBinContent(kGlobalBin);
                else
                {
                    cout << "[WARNING] No Syst histogram found" << endl;
                    return fResult;
                }
...
template <class Tclass>
std::vector<float>
fEvaluateExtrapolationUncertainty(bool fIsConditional, Tclass *fStaticUncertaintySpectrum, Tclass *fVariableUncertaintySpectrum, Float_t fIntegral, Double_t fMaximumFitRange = fMaxPT1D, Double_t fMinimumFitRange = fMinPT1D, TString fName = "", TString fFolder = "", Int_t kEvaluationCycles = kStatEvalCycles)
{
    //
    //  Result container
    ...
    cDrawResults->Write();
    if (fIsConditional)
        cDrawResults->SaveAs(Form("%s/2D/ErrorFits_%s.pdf", fFolder.Data(), fName.Data()));
    else
        cDrawResults->SaveAs(Form("%s/1D/ErrorFits_%s.pdf", fFolder.Data(), fName.Data()));
    delete cDrawResults;
    //
    return fResult;
}
...
template <class Tclass>
std::vector<float>
fEvaluateExtrapolationUncertainty(bool fIsConditional, Tclass *fStaticUncertaintySpectrum, Tclass *fVariableUncertaintySpectrum, std::vector<std::pair<TF1 *, std::vector<float>>> fSystFunc, Double_t fMaximumFitRange = fMaxPT1D, Double_t fMinimumFitRange = fMinPT1D, TString fName = "", TString fFolder = "")
{
    //
    //  Result container
    //  Format: [0] Stat Extrap Unc //! TODO: update
    ...
    for (auto iFuncRange : fSystFunc)
    {
        ...
        iFunc->SetLineColor(uGetRainbowColor(iTer, true));
        //
...
template <class Tclass>
Double_t *
fExtrapolateModel(bool fIsConditional, Tclass *gStatistics, Tclass *gSystematics, Double_t fIntegral = 0.032, TString fName = "ExtrapolateSignal", Double_t fMaximumFitRange = fMaxPT1D, Double_t fMinimumFitRange = fMinPT1D, TString fFolder = "")
{
    //  Optimisation mode
    gROOT->SetBatch(true);
    //
    ...
    //  Graphical Check
    if (fIsConditional)
        cDrawResult->SaveAs(Form("%s/2D/EvaluationFit_%s.pdf", fFolder.Data(), fName.Data()));
    else
        cDrawResult->SaveAs(Form("%s/1D/EvaluationFit_%s.pdf", fFolder.Data(), fName.Data()));
    delete cDrawResult;
    //
...
Double_t fEvaluateINELgt0(Int_t iMultBin, TH1 *hMultCounter, Bool_t k201517 = true)
{
    ...
    for (Int_t iTer = 1; iTer <= kUtilCount->GetNbinsX(); iTer++)
    {
        ...
        if (kBinCenter < kTriggerEffMBins[0] || kBinCenter > kTriggerEffMBins[kTriggerEffMNBins])
            continue;
    ...
    if (iMultBin < 0 || iMultBin > nBinMult)
    {
        for (Int_t iTer = 0; iTer < nBinMult; iTer++)
            fResult += fEvaluateINELgt0(iTer, kUtilCount);
    }
    else
    {
...
Double_t
fEvaluateINELgt0
 ( Int_t iMultBin, TH1  *hMultCounter, Bool_t k201517 = true )  {
    //!
    //! Define result
    auto    fResult = -1;
    auto    kUtilCount  =   (TH1F*)(hMultCounter->Clone());
    //! Loop over reference histogram
    for ( Int_t iBin = 1; iBin <= kUtilCount->GetNbinsX(); iBin++ ) {
        auto    iBinCenter  =   kUtilCount->GetBinCenter(iBin);
        //! If the bin is outside efficiency limits, discard
        if ( iBinCenter < kTriggerEffMBins[0] || iBinCenter > kTriggerEffMBins[kTriggerEffMNBins] ) continue;
        auto    iBinContent =   kUtilCount->GetBinContent(iBin);
        //! Loop over efficiency bins
        for ( Int_t iTer = 1; iTer <= kTriggerEffMNBins; iTer++ )  {
            if ( iBinCenter < kTriggerEffMBins[iTer] )   {
                auto CurrentEfficiency = kLHC15n_fEV*kTriggerEffM15n[iTer-1] + kLHC17pq_fEV*kTriggerEffM17pq[iTer-1];
                if ( !k201517 ) CurrentEfficiency= 1.;
                kUtilCount->SetBinContent(iTer,iBinContent/CurrentEfficiency);
                break;
            }
        }
    }
...
void fSetPhiCandidate(TTree *TPhiCandidate, Struct_PhiEfficiency &evPhiEfficiency)
{
    ...
    TPhiCandidate->SetBranchAddress("Spherocity", &evPhiEfficiency.Spherocity);
...
Bool_t fSetCandidates(TTree *TPhiCnd, Struct_PhiEfficiency &fTargetPhi, TTree *TKaonCn, Struct_KaonEfficiency &fTargetKaon)
{
    if (!TPhiCnd && !TKaonCn)
        return false;
    if (TPhiCnd)
        fSetPhiCandidate(TPhiCnd, fTargetPhi);
    if (TKaonCn)
        fSetKaonCandidate(TKaonCn, fTargetKaon);
    return true;
}
//
//_____________________________________________________________________________
...
std::pair<std::vector<float>, TH1F *> uIntegralError(std::vector<TH1F *> hTargetStat, std::vector<TH1F *> hTargetSyst)
{
    ...
    for (Int_t iTer = 0; iTer < kStatEvalCycles; iTer++)
    {
        ...
        for (auto hHistogram : fNewTarget)
        {
            ...
            gPad->SetLogy();
            if (iTer == 0)
                fSumErrors(hTargetStat.at(jTer), hTargetSyst.at(jTer))->Draw("SAME");
            fLevyTsallis->DrawCopy("same");
            if (iTer == kStatEvalCycles - 1)
                fSumErrors(hTargetStat.at(jTer), hTargetSyst.at(jTer))->Draw("SAME");
            //
            jTer++;
        }
        ...
        gPad->SetLogy();
        if (iTer == 0)
            hConditionalYieldTest->Draw("SAME");
        fLevyTsallis->DrawCopy("same");
        if (iTer == kStatEvalCycles - 1)
            hConditionalYieldTest->Draw("SAME");
        //
...
void fCalculateSystematics()
{
}

void uOrderPTCandidates(Struct_PhiCandidate &fCurrent_Candidates, Bool_t kBiggerFirst = false)
{
    auto kContinue = true;
    Int_t kNewArray[128];
    ...
    while (kContinue)
    {
        ...
        for (Int_t iPhi = 0; iPhi < fCurrent_Candidates.nPhi - 1; iPhi++)
        {
            ...
            if (kBiggerFirst && (kPT_Cand1 >= kPT_Cand2))
                continue;
            if (!kBiggerFirst && (kPT_Cand1 <= kPT_Cand2))
                continue;
...
void uPlotInvMass(TH2F *hTarget, TString kFolder)
{
    ...
    cDrawHisto->SaveAs(Form("%s/InvariantMass_%.1f_%.1f_%.1f_%.1f.eps", kFolder.Data(), fArrPT2D[iHisto], fArrPT2D[iHisto + 1], fArrPT2D[jHisto], fArrPT2D[jHisto + 1]));
    delete cDrawHisto;

    gROOT->SetBatch(kFALSE);
}
void uPlotInvMass(TH1F *hTarget, TString kFolder)
{
    auto kTokenArray = TString(hTarget->GetName()).Tokenize("_");
    auto kLast = (TObjString *)kTokenArray->At(kTokenArray->GetLast());
    ...
    uLatex->SetTextSize(0.04);
    if (is_pp_anl)
        uLatex->DrawLatexNDC(0.12, 0.90, Form("pp  #sqrt{#it{s}}= %2.2f TeV", kEnergy));
    if (is_pb_anl)
        uLatex->DrawLatexNDC(0.12, 0.90, Form("pPb #sqrt{#it{s}}= %2.2f TeV", kEnergy));
    uLatex->DrawLatexNDC(0.12, 0.85, "#phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5");
    //
...
TCanvas *
uPlotCorrelation(TH1F *hStatTarget, TH1F *hSystTarget, TString kFolder)
{
    //
    //  Batch mode ON
    gROOT->SetBatch(kTRUE);
    //
    //  --- X axis
    hStatTarget->GetXaxis()->SetLabelOffset(0.015);
    hStatTarget->GetXaxis()->SetTitleOffset(1.25);
    hStatTarget->GetXaxis()->SetNdivisions(8);
    hStatTarget->GetXaxis()->SetTitle(Form("#Delta#phi"));
    //  --- Y axis
    hStatTarget->GetYaxis()->SetTitleOffset(1.75);
    hStatTarget->GetYaxis()->SetNdivisions(8);
    hStatTarget->GetYaxis()->SetTitle(Form("d#it{N}_{#phi#phi}^{2}/d#varphi d#it{y} (x10^{6})"));
    //  --- General Style
    hStatTarget->Scale(1e6);
    hStatTarget->SetMarkerStyle(uGetMarker(5));
    hStatTarget->SetMarkerColor(uGetColor(1));
    hStatTarget->SetMarkerSize(2);
    hStatTarget->SetLineColor(uGetColor(1));
    hStatTarget->SetFillColorAlpha(uGetColor(1), 0.);
    //
    SetStyle();
    ...
    TCanvas *cDrawHisto = new TCanvas("cDrawHisto", "cDrawHisto", 3000, 3000);
    //
    hStatTarget->Draw("SAME PE");
    //
    uLatex->SetTextFont(60);
    uLatex->SetTextSize(0.05);
    uLatex->DrawLatexNDC(0.55, 0.83, "ALICE");
    uLatex->SetTextFont(42);
    uLatex->SetTextSize(0.04);
    if (is_pp_anl)
        uLatex->DrawLatexNDC(0.55, 0.78, Form("pp  #sqrt{#it{s}}= %2.2f TeV", kEnergy));
    if (is_pb_anl)
        uLatex->DrawLatexNDC(0.55, 0.78, Form("pPb #sqrt{#it{s}}= %2.2f TeV", kEnergy));
    uLatex->DrawLatexNDC(0.55, 0.73, "#phi #rightarrow K^{+}K^{-}, |#it{y}|<0.5");
    //
    cDrawHisto->SaveAs(Form("%s/Correlation.pdf", kFolder.Data()));
    cDrawHisto->SaveAs(Form("%s/Correlation.eps", kFolder.Data()));
    //
    //  Batch mode OFF
    gROOT->SetBatch(kFALSE);
    return cDrawHisto;
...
//
template <Bool_t kQuadratic = true>
std::tuple<Float_t, Float_t, Float_t>
uCalculateSimpleRatio(std::tuple<Float_t, Float_t, Float_t> Yield1tuple, std::tuple<Float_t, Float_t, Float_t> Yield2tuple)
{
    std::tuple<Float_t, Float_t, Float_t> kResult;
    //  Mean Value
    get<0>(kResult) = get<0>(Yield2tuple) / (get<0>(Yield1tuple));
    //  Statistical Error
    get<1>(kResult) = kQuadratic ? get<0>(kResult) * SquareSum({get<1>(Yield1tuple) / get<0>(Yield1tuple), get<1>(Yield2tuple) / get<0>(Yield2tuple)}) : get<0>(kResult) * (get<1>(Yield1tuple) / get<0>(Yield1tuple) + get<1>(Yield2tuple) / get<0>(Yield2tuple));
    //  Systematical Error
    get<2>(kResult) = kQuadratic ? get<0>(kResult) * SquareSum({get<2>(Yield1tuple) / get<0>(Yield1tuple), get<2>(Yield2tuple) / get<0>(Yield2tuple)}) : get<0>(kResult) * (get<2>(Yield1tuple) / get<0>(Yield1tuple) + get<2>(Yield2tuple) / get<0>(Yield2tuple));
    return kResult;
}
//
template <Bool_t kQuadratic = true>
std::tuple<Float_t, Float_t, Float_t>
uCalculateSquareRatio(std::tuple<Float_t, Float_t, Float_t> Yield1tuple, std::tuple<Float_t, Float_t, Float_t> Yield2tuple)
{
    std::tuple<Float_t, Float_t, Float_t> kResult;
    //  Mean Value
    get<0>(kResult) = get<0>(Yield2tuple) / (get<0>(Yield1tuple) * get<0>(Yield1tuple));
    //  Statistical Error
    get<1>(kResult) = kQuadratic ? get<0>(kResult) * SquareSum({2 * get<1>(Yield1tuple) / get<0>(Yield1tuple), get<1>(Yield2tuple) / get<0>(Yield2tuple)}) : get<0>(kResult) * (2 * get<1>(Yield1tuple) / get<0>(Yield1tuple) + get<1>(Yield2tuple) / get<0>(Yield2tuple));
    //  Systematical Error
    get<2>(kResult) = kQuadratic ? get<0>(kResult) * SquareSum({2 * get<2>(Yield1tuple) / get<0>(Yield1tuple), get<2>(Yield2tuple) / get<0>(Yield2tuple)}) : get<0>(kResult) * (2 * get<2>(Yield1tuple) / get<0>(Yield1tuple) + get<2>(Yield2tuple) / get<0>(Yield2tuple));
    return kResult;
}
//
template <Bool_t kQuadratic = true>
std::tuple<Float_t, Float_t, Float_t>
uCalculateSigmaParameter(std::tuple<Float_t, Float_t, Float_t> Yield1tuple, std::tuple<Float_t, Float_t, Float_t> Yield2tuple)
{
    std::tuple<Float_t, Float_t, Float_t> kResult;
    get<0>(kResult) = 2 * get<0>(Yield2tuple) + get<0>(Yield1tuple) - get<0>(Yield1tuple) * get<0>(Yield1tuple);
    get<1>(kResult) = kQuadratic ? SquareSum({2 * get<1>(Yield2tuple), (-1 + 2 * get<0>(Yield1tuple)) * get<1>(Yield1tuple)}) : 2 * get<1>(Yield2tuple) + (-1 + 2 * get<0>(Yield1tuple)) * get<1>(Yield1tuple);
    get<2>(kResult) = kQuadratic ? SquareSum({2 * get<2>(Yield2tuple), (-1 + 2 * get<0>(Yield1tuple)) * get<2>(Yield1tuple)}) : 2 * get<2>(Yield2tuple) + (-1 + 2 * get<0>(Yield1tuple)) * get<2>(Yield1tuple);
    return kResult;
}
//
template <Bool_t kQuadratic = true>
std::tuple<Float_t, Float_t, Float_t>
uCalculateGammaParameter(std::tuple<Float_t, Float_t, Float_t> Yield1tuple, std::tuple<Float_t, Float_t, Float_t> Yield2tuple)
{
    std::tuple<Float_t, Float_t, Float_t> kResult;
    get<0>(kResult) = 2 * get<0>(Yield2tuple) / get<0>(Yield1tuple) - get<0>(Yield1tuple);
    get<1>(kResult) = kQuadratic ? get<0>(kResult) * SquareSum({2 * get<1>(Yield1tuple) / get<0>(Yield1tuple), get<1>(Yield2tuple) / get<0>(Yield2tuple)}) : get<0>(kResult) * (2 * get<1>(Yield1tuple) / get<0>(Yield1tuple) + get<1>(Yield2tuple) / get<0>(Yield2tuple));
    get<2>(kResult) = kQuadratic ? get<0>(kResult) * SquareSum({2 * get<2>(Yield1tuple) / get<0>(Yield1tuple), get<2>(Yield2tuple) / get<0>(Yield2tuple)}) : get<0>(kResult) * (2 * get<2>(Yield1tuple) / get<0>(Yield1tuple) + get<2>(Yield2tuple) / get<0>(Yield2tuple));
    return kResult;
}
//
//
//  Final Quantities Calculation
template <Bool_t kQuadratic = true>
std::map<TString, std::tuple<Float_t, Float_t, Float_t>>
uCalculateDerivedQuantities(std::tuple<Float_t, Float_t, Float_t> Yield1tuple, std::tuple<Float_t, Float_t, Float_t> Yield2tuple)
{
    std::map<TString, std::tuple<Float_t, Float_t, Float_t>> kMapResult;
    //  Bookeeping, log input
    kMapResult["N1"] = Yield1tuple;
    kMapResult["N2"] = Yield2tuple;
    //  Ratios
    kMapResult["R1"] = uCalculateSimpleRatio<kQuadratic>(Yield1tuple, Yield2tuple);
    kMapResult["R2"] = uCalculateSquareRatio<kQuadratic>(Yield1tuple, Yield2tuple);
    //  Parameters
    kMapResult["P1"] = uCalculateSigmaParameter<kQuadratic>(Yield1tuple, Yield2tuple);
    kMapResult["P2"] = uCalculateGammaParameter<kQuadratic>(Yield1tuple, Yield2tuple);
    //  Result
    return kMapResult;
}
//
//  Automatic final quantities histogram
template <Bool_t kQuadratic = true,
          Bool_t kSystmatic = false>
TH1F *
uPlotDerivedQuantitiesRaw(std::tuple<Float_t, Float_t, Float_t> Yield1tuple, std::tuple<Float_t, Float_t, Float_t> Yield2tuple)
{
    //  Calculate Results
    std::map<TString, Int_t> kLegend = {{"N1", 1}, {"N2", 2}, {"R1", 3}, {"R2", 4}, {"P1", 5}, {"P2", 6}};
    auto kMapResult = uCalculateDerivedQuantities<kQuadratic>(Yield1tuple, Yield2tuple);
    //  Histogram creation
    auto kPlotResult = new TH1F("", "", 6, 0.5, 6.5);
    auto iTerator = 0;
    for (auto [kName, kValue] : kMapResult)
    {
        iTerator++;
        kPlotResult->SetBinContent(kLegend[kName], get<0>(kValue));
        kSystmatic ? kPlotResult->SetBinError(kLegend[kName], get<2>(kValue)) : kPlotResult->SetBinError(kLegend[kName], get<1>(kValue));
    }
    return kPlotResult;
}
//
